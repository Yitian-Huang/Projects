...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
    double resizeAlpha, const Prober& prober, const Hasher& hash, const KEqual& kequal)
       :  hash_(hash), kequal_(kequal), prober_(prober), table_(11, nullptr)
{
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    if((double)totalelement/CAPACITIES[mIndex_]>=alpha)
    {
        resize();
        loc = probe(p.first);
    }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    std::vector<HashItem*> temp2(CAPACITIES[mIndex_], nullptr);
    table_ = temp2;
    totalelement = 0;
    numelement = 0;
    for(HashItem* i : temp)
    {
        if(i!=nullptr && i->deleted==false)
            insert(i->item);
        delete i;
    }    
