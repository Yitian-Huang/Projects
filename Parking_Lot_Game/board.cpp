...
Board::Board(const Board& b) :
    vehicles_(b.vehicles_), grid_(b.grid_), myst(b.myst)
{
}
...
bool Board::move(VID_T vid, int amount)
{
    if(isLegalMove(vid, amount)) {
        myst.push(make_pair(vid,amount));
...
{
    MovePair lastmove = this->myst.top();
    this->myst.pop();
    move(lastmove.first, -lastmove.second);
    myst.pop();
}
...
{
    MovePairList retlist;
    VehicleMap::const_iterator it;
    for(it = this->vehicles_.begin(); it!=this->vehicles_.end(); ++it)
    {
        VID_T id = it->first;
        int i = -1;
        while(this->isLegalMove(id, i))
        {
            retlist.push_back(make_pair(id, i));
            i--;
        }
        i=1;
        while(this->isLegalMove(id, i))
...
bool Board::operator<(const Board& other) const
{
    ...
    unsigned int size2 = other.size();
    for(unsigned int i =0; i< size1;i++)
    {
        for(unsigned int j=0; j< size1; j++)
        {
            board1+=this->grid_[i][j];
        }
    }
